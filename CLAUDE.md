...
- ✅ **R2 코드 완료(라이브 스모크 대기)** — 캐논 생성 end-to-end: `prompts.ts` + `research.ts`(song_research 캐시) + `generate.ts`(3-role 캐논→게이트→`canonical_tones`) + `json.ts` + `rest.sbInsert`. **캐논 게이트를 스키마+base_gear 모양으로 축소, gear KB 실존은 투영으로 이관**(부트스트랩 닭-달걀 해소). **미완: Gemini 실연결 스모크** — `web/.env.local`에 `LLM_PROVIDER`/`GEMINI_API_KEY` 필요(env엔 아직 Supabase 키만).
- ✅ **R3 코드 완료(시드 재실행 대기)** — `projector.ts`: base_gear 역인덱스(`effects_catalog.entries`, md "(기반:)" 추출) + **2단 룩업**(정확→경계 포함) + kind 교차검증 + 대표 파트(lead→backing→solo) real-amp(CAB off)/phone(CAB on) 파생 + 게이트 + `tones` 적재. 라운드트립 골든 게이트(PATCHES 91블록: 84 매핑·mismatch 0·예외 6종 명시). 테스트 전체 377 그린. **미완: 리모트 processors 시드 재실행**(`npx tsx scripts/seed-reset.ts` — entries 반영 전엔 projectSong 이 fail-fast). 다음 = **R4**(웹 개편: 생성 폼+5탭+카탈로그). 단계 전체 = 설계 §6.
