...
정석 마샬 크런치. **미드 푸시가 핵심**(Mid 7). 리듬은 다운스트로크로 단단하게, 솔로는 게인을 더 올리기보다 **Green OD 부스트로 미드를 밀어** 서스테인을 얻는 게 노엘 톤에 가깝다.
...
switching: {"A":"솔로 — Green OD + Slapback ON (Plexi는 더 세게 부스트)"}
...
- **모듈 매핑 (GP-150).** Green OD은 독립 "OD 모듈"이 아니라 **DST 모듈의 오버드라이브 모델**이다(GP-150엔 OD 슬롯이 따로 없음). 클린 게인 부스트가 필요하면 **PRE 모듈**의 Micro Boost/Boost를 쓴다. 웹 체인의 `[DST] 오버드라이브 · Green OD` 표기가 이 뜻 — 모듈(DST) · 효과종류(오버드라이브) · 모델(Green OD).
- **Marshall이 정답.** UK 800(JCM800)이 80~90년대 브릿팝 크런치의 정점이라 리듬·솔로 모두 커버 → 정석. 더 빈티지·개방적이면 UK SLP(Plexi), 합주 자리잡기엔 UK 900 + 미드 EQ.
- **솔로는 게인보다 부스트.** Green OD로 미드를 밀어 서스테인을 얻는 게 노엘 톤에 가깝다(특히 Plexi).
