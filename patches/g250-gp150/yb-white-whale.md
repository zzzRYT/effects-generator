...
합주에서 묻히지 않게 미드를 민 록 지향 해석. 베이스를 **UK 900(Marshall JCM900)** 크런치로 깔고, 벌스는 넥 싱글 + 기타 볼륨을 7쯤으로 줄여 정리한다(앰프단에서 자연스럽게 클린에 가깝게). 후렴은 풋스위치 A로 **Green OD(TS-808)**을 밟아 저역을 타이트하게 조이고 게인을 밀며, 솔로는 B의 부스트로 들어 올린다. 공간계는 합주에서 과하지 않게 얕게.
...
switching: {"A":"후렴 — Green OD(TS-808) ON (타이트 푸시)","B":"솔로 — Micro Boost ON (리드 부스트)"}

## Variation: 앰비언트 — JC-120 + Shimmer (더 꿈결 같은 해석)
공간감을 극대화한 버전. **J-120 CL(Roland JC-120)**의 투명한 클린에 옅은 **G-Chorus**를 상시로 깔아 벌스 아르페지오를 더 넓고 차갑게 띄운다. 딜레이는 점8분으로 길게, 리버브는 **Shimmer**로 옥타브 잔향을 더해 앰비언트 질감. 후렴은 풋스위치 A의 **Blues OD**로 드라이브를 얹고(JC 클린만으론 클라이맥스가 안 터지므로), 솔로는 B의 **Boost(EP Booster, 트레블 부스터 역할)**로 밀어 올린다.
...
switching: {"A":"후렴 — Blues OD ON (드라이브)","B":"솔로 — Boost(EP Booster, 트레블 부스터 역할) ON (리드 부스트)"}
