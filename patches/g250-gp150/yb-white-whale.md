...
→ 모든 변주는 **풋스위치 A = 후렴 드라이브**, **풋스위치 B = 클라이맥스 부스트(A 위에 얹음)**로 같은 손동작을 유지한다. 벌스는 넥 싱글(따뜻한 클린), 후렴·솔로는 브릿지 험버커(두께·서스테인)로 픽업을 바꾼다.
...
guitar: {"selector":5,"volume":10,"note":"벌스 넥 싱글(pos 5) 클린 / 후렴·솔로 브릿지 험버커(pos 1)"}
switching: {"A":"후렴 — Force ON (크런치)","B":"클라이맥스 솔로 — Micro Boost ON (+Force 위에 얹어 부스트)"}

## Variation: 합주용 — JCM900 게인 빌드 (미드 푸시)

합주에서 묻히지 않게 미드를 민 록 지향 해석. 베이스를 **UK 900(Marshall JCM900)** 크런치로 깔고, 벌스는 넥 싱글 + 기타 볼륨을 7쯤으로 줄여 정리한다(앰프단에서 자연스럽게 클린에 가깝게). 후렴은 풋스위치 A로 **Green OD**을 밟아 저역을 타이트하게 조이고 게인을 밀며, 솔로는 B의 부스트로 들어 올린다. 공간계는 합주에서 과하지 않게 얕게.
체인: 기타 → OD(Green OD, FS A) → Boost(Micro, FS B) → AMP(UK 900) → CAB(UK 30) → Delay → Reverb(Room).
...
guitar: {"selector":5,"volume":10,"note":"벌스 넥 싱글(pos 5)+볼륨 7 정리 / 후렴·솔로 브릿지 험버커(pos 1) 볼륨 풀"}
switching: {"A":"후렴 — Green OD ON (타이트 푸시)","B":"솔로 — Micro Boost ON (리드 부스트)"}

## Variation: 앰비언트 — JC-120 + Shimmer (더 꿈결 같은 해석)

공간감을 극대화한 버전. **J-120 CL(Roland JC-120)**의 투명한 클린에 옅은 **G-Chorus**를 상시로 깔아 벌스 아르페지오를 더 넓고 차갑게 띄운다. 딜레이는 점8분으로 길게, 리버브는 **Shimmer**로 옥타브 잔향을 더해 앰비언트 질감. 후렴은 풋스위치 A의 **Blues OD**로 드라이브를 얹고(JC 클린만으론 클라이맥스가 안 터지므로), 솔로는 B의 **Boost**로 밀어 올린다.
...
[
  ...
  {"type":"PRE","category":"BOOST","model":"Boost","base_gear":"Xotic EP Booster","enabled":false,"footswitch":"B",
...
guitar: {"selector":5,"volume":10,"note":"넥 싱글(pos 5) 거의 고정 / 솔로만 브릿지 험버커(pos 1)"}
switching: {"A":"후렴 — Blues OD ON (드라이브)","B":"솔로 — Boost ON (리드 부스트)"}
