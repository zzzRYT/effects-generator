...
- **🎨 디자인 수렴 트랙 개시 (2026-07-19)** — 브랜치 `feat/toneforge-convergence`. 북극성 = `docs/goal-toneforge-convergence.md` (Tone Forge 디자인 `ToneForgeRig.dc.html`로 점진 수렴). 사이클 #8~#11 아래 표. 확정 fork: 하이브리드 색(앰버 섀시+타입별 hue), 첫 사이클=수직 슬라이스(토큰+Patch). **🎉 수렴 트랙 #8~#11 전부 ✅ 완료.** 4개 화면(Patch·Catalog·Request·Footer) 전부 Tone Forge 하드웨어 룩으로 수렴. 전 게이트 green(255 vitest·188 Playwright·lint/tsgo/tsc/build). 남은 폴리시: 변주 탭 위젯 앰버화(현재 블루 잔존), Lighthouse 정식 측정. 다음 대형 항목 = #7 `multi-processor-view`(계획됨) 또는 origin push/배포.
...
| 9 | **카탈로그 리스킨** | `catalog-reskin` | ✅ (수렴) 홈(SongIndex)→TFCatalog 룩: Oswald 대문자 타이틀 · 패치카드 행(융기 섀시+앰버 아티스트 라벨) · 앰버 활성칩(글로우)+리세스 검색 웰 · 앰버 포커스. CSS 전용(마크업/필터 로직 불변). **188 Playwright green(홈+dialog 스냅샷 갱신), lint/tsgo/tsc green.** |
| 10 | **제보=Generate 리스킨** | `generate-as-request` | ✅ (수렴) 제보 폼+/request 페이지→TFGenerate 룩: Oswald 대문자 타이틀 · Space Mono 대문자 라벨(빨강 필수마크) · 리세스 입력 웰 · 글로우 그린 LED 전송 버튼 · 앰버 트리거/백링크. 제출은 여전히 이메일(Web3Forms). CSS 전용. |
| 11 | **모바일 셸 + 브랜드** | `mobile-shell` | ✅ (수렴) Footer 리스킨(앰버 CTA·상단 하이라이트 엣지) + 브랜드 결정: "Tone Forge"(placeholder) 미채택, 현행 "GP-150 톤 라이브러리" 유지(Oswald 대문자 통일). ⚠ sticky 풋스위치 바는 read-only 앱엔 불필요(디자인 1e 장식)로 드롭. |
