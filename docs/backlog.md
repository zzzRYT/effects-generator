...
## 🧭 북극성 로드맵 (2026-07-17) — 현행

> **읽기 순서: 이 블록이 유일한 현행.** 아래 캐논·투영 부활(2026-07-06) 블록의 R0~R4.5 완료 기록은 유효하나,
> **잔여 단계(R5~R7)의 순서·범위는 이 블록이 대체**한다. 근거 = dev-hub wayfinder 맵
> `공개-제품-북극성-map`(결정 티켓 EG-T1~T9) + `CLAUDE.md` "북극성" 섹션.
>
> 요지: 존재 이유 = **공개 제품/서비스**('개인용 우선' 폐기). 타깃 = **입문자(저가 멀티이펙터)**.
> 핵심 가치 = "이 곡 + 내 기기 → 바로 걸 수 있는 패치". 품질 최소 증거 = 본인 GP-150 독푸딩.
> 순서 = **독푸딩 → 품질 환류 → 출시 준비 → 공개**.
>
> **단계:**
> - **D1 독푸딩 라운드 1** — 프로토콜 v1(EG-T1): 10곡(치고 싶은 곡 우선 + 클린/크런치/하이게인 각 최소 2곡),
>   회차 20~30분·주 2~3곡, 곡당 **종합 5점 + 문제 모듈 태그**(AMP/DST/CAB/DLY/RVB/MOD/EQ/없음) + 한줄 메모.
>   기록 = `docs/dogfooding/round-1.md`(마크다운 테이블: 날짜·곡·role·점수·태그·메모).
>   **라운드 완주 전 파이프라인(프롬프트·투영) 동결** — 중간 수정 = 표본 오염.
>   baseline(현행 텍스트 생성)만 사용 — R4.5 오디오 관측은 유보(EG-T4).
> - **D2 품질 환류** — 라운드 1 완주 후 일괄 분석: 모듈별 실패 패턴 → 프롬프트·투영 매핑 개선 → 저점 곡 재생성으로
>   라운드 2. **R4.5 재진입 조건**: 저점 곡 문제가 톤 정체성(AMP/DST/CAB)에 몰리면 라운드 2 저점 곡 한정
>   enriched **귀 A/B**(채택 판정 = 귀 A/B 명확 우세 + 투영 실패율 증가 없음, 설정표 게이트는 참고 지표).
>   **공개 게이트 = 최종 라운드 평균 ≥3.5 & 4점+ 곡 ≥60% & 1점 0개** — 미통과 시 출시 보류.
> - **D3 출시 준비** (게이트 통과 후, 항목 간 병렬 가능) —
>   ① **GE-150 온보딩**(EG-T8): 매뉴얼 PDF FX 표("(기반:)" 표기 있음) → GP-150과 동일한 스크립트 경로로
>   `processors.effects_catalog` 시드 → 카탈로그 검증(스키마·FX 실존·노브 범위) → 투영·렌더 확인. 구 R7의 실체화.
>   노브 정의 불완전 판명 시 Sonicake QME-200으로 교체(EG-T8 되돌릴 조건).
>   ② **i18n 3언어**(en/ko/ja) — UI 전면.
>   ③ **`/gear/<slug>` 기기 페이지**(GP-150·GE-150 2장, 모듈→base_gear 매핑 전시) + **톤 상세 블록별 base_gear
>   라벨** + 기본 SEO(SSG·메타/구조화 데이터·sitemap·hreflang) = 출시 요건, 그 이상은 출시 후(EG-T9).
>   ④ **생성 가드 3종**(EG-T5) — Gemini 일일 비용 상한(전역 캡)·IP/세션 rate limit·경량 봇 방지(honeypot 수준).
>   ⑤ **곡 20~30 선적재** — 독푸딩 10곡 시드 + 영어권 입문 록·J-pop 보강.
> - **D4 공개** — 정식 오픈(검색·커뮤니티 유입 허용), 계정 없음(익명). 이후 곡 축적 = 유저 생성 캐시 구조.
>
> **강등·유보** (재승격 트리거 = 각 결정 티켓의 '되돌릴 조건'):
> - ~~R5 어드민 입력 UI~~ → **출시 후 강등**(EG-T6). 온보딩·교정은 스크립트 경로가 공식 도구(수동 온보딩·크론 없음
>   원칙 유지). 재승격 = 온보딩 월 1회+ 또는 어드민 증원.
> - **R4.5 오디오 랩 = 유보**(EG-T4). 코드·`/lab/audio-tone`·자동 게이트 보존, 표본 수집·실 smoke 중단. 재진입 = D2.
> - R6 요청 폼 확장(`request-form-v2`)은 유지하되 D3와 독립(우선순위는 출시 준비 뒤).
> - 계정·저장·공유 캐시·수익화·베타 채널 = 출시 후 신호 기반 재검토(맵 fog 항목).

## 🔁 캐논·투영 부활 (2026-07-06) — R0~R4.5 기록 유효 · 잔여 단계는 위 북극성 로드맵이 대체

> ~~**읽기 순서: 이 블록이 유일한 현행.**~~ (2026-07-17 — 현행은 위 북극성 로드맵. 이 블록의 R0~R4.5 완료 기록과
> 아키텍처 결정은 유효, R5~R7 순서·범위만 대체됨.) 아래 구조 리셋(2026-07-04)·캐논·투영(2026-06-28)·피벗(2026-06-25) 블록은 **폐기 또는 부분 부활** — 기록용 보존, 각 블록 상단 노트 참조.
...
> (계정/공유캐시/정규화 깔때기는 이번 범위 밖 — 2026-07-17 '개인용 우선'은 폐기, 공개 제품 북극성으로 대체).
>
> **단계:**
...
> - ~~**R5** — 어드민 — gear/processors/guitars 수동 입력 UI + 레퍼런스 업로드(Storage).~~ (2026-07-17 출시 후
>   강등 — EG-T6. 온보딩·교정은 스크립트 경로가 공식 도구.)
> - **R6** — 요청 폼 확장(별도 `/superpowers:brainstorm` 사이클, 아래 표 `request-form-v2` 참조). (2026-07-17 —
>   유지하되 출시 준비 D3 뒤로.)
> - ~~**R7** — 둘째 기기 검증 — 실제 멀티이펙터 1종 수동 온보딩→투영→렌더(비전 증명).~~ (2026-07-17 — 북극성
>   로드맵 D3-①(GE-150 온보딩)로 실체화·흡수.)
