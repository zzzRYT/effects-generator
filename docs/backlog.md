...
- ▶ **다음: 사이클 #4 `request-form`** — `/superpowers:brainstorm request-form`부터. 곡 제보 폼(곡·아티스트·요청자·메모) → 폼-투-이메일(Web3Forms 권장, access key 는 `web/.env`) → Gmail. **백엔드 0.** 마지막 사이클(가치 보여준 뒤 제보). 선행 권장: `cd web && npx playwright install`.
