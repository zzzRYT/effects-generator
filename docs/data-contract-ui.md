...
4. **색만으로 의미 전달 금지** — 블록은 색 + **모듈 약어 배지**(DST/PRE/AMP/CAB/DLY/RVB…) 병기. `category` 있으면 효과종류 라벨(오버드라이브/부스트/퍼즈/컴프…)을 모델명 옆에 함께 표기해 "어느 모듈의 무슨 효과인지"를 텍스트로도 전달. (edge-3.11) — 예: `[DST] 오버드라이브 · TS-808`.

타입(모듈) → 토큰 그룹 매핑. `type`은 GP-150 실제 12모듈이고, **드라이브 계열은 `category`로 색을 정한다**(PRE/DST가 효과종류에 따라 갈리기 때문 — `docs/parser-contract.md` "모듈 ↔ 효과"):

| 조건 | 토큰 |
|------|------|
| `category` ∈ `OD` `DST` `FUZZ` `BOOST` (드라이브/게인 계열) | `--color-od` |
| `category` ∈ `FILTER` `PITCH` | `--color-mod` |
| `category` = `COMP` | `--color-util` |
| `type` = `AMP` | `--color-amp` |
| `type` = `CAB` (또는 `NS`/SnapTone) | `--color-cab` |
| `type` = `DLY` | `--color-dly` |
| `type` = `RVB` | `--color-rvb` |
| `type` ∈ `MOD` `WAH` | `--color-mod` |
| `type` ∈ `NR` `EQ` `VOL` `PRE`(category 없음) · `DST`(category 없음 → od) | `--color-util` / `--color-od` |

> 해석 순서: **category 우선 → 없으면 type**. category 있는 PRE/DST 블록은 category가 색을 결정하고, 단일 의미 모듈은 type이 결정한다. 단일 출처는 `lib/blockType.ts`.
