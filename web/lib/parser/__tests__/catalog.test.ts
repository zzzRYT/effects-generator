...
`;
  ...
  it("카탈로그에 있는 model 은 통과한다", () => {
    const { song, errors } = parsePatch(patch("Green OD"), "patches/g250-gp150/t.md", undefined, options);
    expect(errors).toHaveLength(0);
    expect(song).not.toBeNull();
  });

  it("base-gear 이름은 model-unknown 에러로 빌드를 막는다", () => {
    const { song, errors } = parsePatch(patch("TS-808"), "patches/g250-gp150/t.md", undefined, options);
